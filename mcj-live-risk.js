...
var API_KEY = "your actual random string";
  ...
  function paintLiveFigures() {
    ...
    if (d.week_pnl !== null) {
      var p = document.getElementById("s-pnl");
      if (p) {
        p.textContent = (d.week_pnl >= 0 ? "+$" : "-$") + fmt0(Math.abs(d.week_pnl));
        p.style.color = d.week_pnl > 0 ? "var(--green)" : d.week_pnl < 0 ? "var(--red)" : "var(--navy)";
      }
    }
  ...
  function mergeTrackerFromLive() {
    if (!LIVE || !LIVE.week || typeof window.getTracker !== "function") return;

    var t = window.getTracker();
    if (!t.days) t.days = [[], [], [], [], []];
